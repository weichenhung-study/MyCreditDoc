...
## 3.建置application(SpringBoot)
### 2.1 信用卡管理服務（management-api）:8085
#### (1) cd到專案dockerfile目錄
	mvn clean package
#### (2) 創建映像檔
    docker build -t springboot-management-api-img .
#### (3) 啟動容器
    docker run -p 8085:8080 --name springboot-management-api --net mysqlnetwork -e MYSQL_HOST=customerinfo-service -e MYSQL_PORT=3306 -e MYSQL_DB_NAME=apdata -e MYSQL_USER=root -e MYSQL_PASSWORD=root springboot-management-api-img

### 2.2 交易服務（transactions-api）::8086
#### (1) cd到專案dockerfile目錄
	mvn clean package
#### (2) 創建映像檔
    docker build -t springboot-transactions-api-img .
#### (3) 啟動容器
    docker run -p 8086:8080 --name springboot-transactions-api --net mysqlnetwork -e MYSQL_HOST=customerinfo-service -e MYSQL_PORT=3306 -e MYSQL_DB_NAME=apdata -e MYSQL_USER=root -e MYSQL_PASSWORD=root springboot-transactions-api-img

### 2.3 帳單服務（billing-api）:8087
#### (1) cd到專案dockerfile目錄
	mvn clean package
#### (2) 創建映像檔
    docker build -t springboot-billing-api-img .
#### (3) 啟動容器
    docker run -p 8087:8080 --name springboot-billing-api --net mysqlnetwork -e MYSQL_HOST=customerinfo-service -e MYSQL_PORT=3306 -e MYSQL_DB_NAME=apdata -e MYSQL_USER=root -e MYSQL_PASSWORD=root springboot-billing-api-img

### 2.4 爭議處理服務（dispute-api）:8088
#### (1) cd到專案dockerfile目錄
	mvn clean package
#### (2) 創建映像檔
